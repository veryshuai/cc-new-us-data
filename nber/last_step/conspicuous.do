...
capture program drop initialize_globals; program define initialize_globals;
	...
	global my_pwd "~/Documents/cc/new_us_data/nber/last_step/";
	global vindices_path "~/Documents/cc/new_us_data/nber/last_step/vindex";
	cd "$my_pwd"; set mem 200m;
#delimit;
	global fandate "20110914"; global fanh "050"; * notice: "050" will translate to h=0.50 etc.;
...
program define merge_vindices
	merge using "${vindices_path}/Vindices20111116_all.dta" /*,unique */
	ta _m*
	drop _m*
end
...
program define vis_lux
	...
	if "$save_vis_lux_vars" == "Yes" {
		...
		keep luxurindexb$spec catsb${subgroup} services ser_catsb${subgroup} luxurindexf$spec ncats${subgroup}
...
*	plots with regression results as a note
	if "$outreg_results" == "Yes" {
		foreach iserv in "" "services" "services ser_catsb${subgroup}" {	
		  foreach ihom in "" { //  " if _n~=11" { // influential analysis just for housing (Hom)	
			...
			if      "`iserv'"=="services" 						local tabser = "ser" 
			else if "`iserv'"=="services ser_catsb${subgroup}" local tabser = "sercatsb"
	...
	if "$reg_Service" == "Yes" reg luxurindexb$spec services [w=luxurindexf$spec] // make sure this specification is comparable to the outreg regressions above.
...
/*
		* these globals are used by vis_lux (but are set outside this program, see below)
  ...
  **The program will loop through grid values of x running from xl to xh;
  ...
  while $cc<=$grid
  {
    ...
    qui replace xg=xg+step;              *update xg;
		qui replace wgt = $wt 
		if $x>xg-(step/2) & $x<=xg+(step/2); *we sum together all the weights of points around the current xg, and this will be the weight for this value-point.; 
    ...
    cap replace py=_b[_cons]+_b[$x]*xg if _n==$cc;
              *this stores predicted value in cc'th row of the data;
    cap replace slope=_b[$x] if _n==$cc;
*		for confindence intervals;
	...
	*list slope in 1/$cc;
	...
	cap merge using Fan\fan${today}${fangr}h$fanh$yname$x; cap ta _m*; cap drop _m*erge;
...
program def run_fan;
	...
	while `i' <=`numcats' {; di `i';
		...
		gen h = "$h" ;
		local grid 30;
		local arg1 fc`i'; //*this was "$yname"`i' in place of arg1 below...but wasn't working... 
		fan `arg1' "$xname" h `grid' xl xh "${yname}" totwt;
		restore;
		local ++i;
	};
end;
...
cap program drop merge_fan; program def merge_fan;
	merge using Fan/fan$fanversion; ta _m*; drop _m*erge;
end;
...
* 	for getting rid of top percentile or other extreme values in case of "fc" or "c";
		...
		if "$yname"=="fc" | "$yname"=="fsgc" {; 
			egen percentiles=cut($yname`i'),group(100); replace random=0 if percentiles==99; };
		if "$yname"=="c" {; 
			egen percentiles=cut($yname`i'),group(100); replace random=0 if percentiles>=98; 
